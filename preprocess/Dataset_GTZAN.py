...
class GTZAN_3s(Dataset):
        ...
        if not os.path.isdir(self._path):
            ...
            if self.fold is None:
                if self.subset == "training":
                    ...
                    self._walker = filtered_train_val
            else:
                self._walker = self.fold
    ...
    def get_labels(self):
        return [mapper_genre[s[:-6]] for s in self._walker]
...
def get_fold_dataloader(data_type, fold, opt:argparse.Namespace):
    ...
    if opt.splits_per_track > opt.total_num_splits:
        opt.splits_per_track = opt.total_num_splits
    return GTZAN_3s(subset=data_type, new_sr=opt.sample_rate, sigma_gnoise=opt.sigma_gnoise, hop_gap=opt.hop_gap,
                        splits_per_track=opt.total_num_splits, fold=fold)
